...
async function createQueue(text) {
...
async function bootSynthesizer(modelKey = null, modelRecord = null) {
  const fallbackKey = 'en_US-libritts_r-medium';
  const resolvedKey = modelKey || fallbackKey;
  const encodedKey = encodeURIComponent(resolvedKey);
  const modelPath = modelRecord?.model_url || `/api/model/${encodedKey}.onnx`;
  const configPath = modelRecord?.config_url || `/api/model/${encodedKey}.onnx.json`;
...
async function emitPreview(text, voice, speed) {
  const stream = activeSynthesizer.stream(await createQueue(text), {
...
async function runSynthesis(text, voice, speed) {
  ...
  const mergedAudio = mergeAudioParts(audioParts);
  self.postMessage({ status: WORKER_EVENT.complete, audio: mergedAudio.toBlob() });
}

function mergeAudioParts(audioParts) {
...
self.addEventListener('message', async ({ data }) => {
  const { type, text, voice, speed, model, modelSpec } = data;

  try {
    if (isWorkerBootstrap(type)) {
      await bootSynthesizer(model, modelSpec);
      return;
    }
