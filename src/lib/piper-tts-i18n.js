...
function coercePhonemizerOutput(sourceText, rawPhonemes) {
  if (typeof rawPhonemes === 'string') {
    return rawPhonemes;
  }

  if (Array.isArray(rawPhonemes)) {
    return rawPhonemes.join(' ');
  }

  if (rawPhonemes && typeof rawPhonemes === 'object') {
    return rawPhonemes.text || rawPhonemes.phonemes || String(rawPhonemes);
  }

  return String(rawPhonemes || sourceText);
}
...
export class SpeechTaskQueue {
  ...
  push(text) {
    const chunks = this.splitText(text) || [text];
...
export class BrowserPiperRuntime {
  ...
  async createPhonemeTimeline(text) {
    ...
    const normalizedText = coercePhonemizerOutput(text, rawPhonemes);
    const sentences = normalizedText.split(/[.!?]+/).filter((segment) => segment.trim());

    return sentences.map((sentence) => Array.from(sentence.trim().normalize('NFD')));
  }

  collectPhonemeIds(phonemeTimeline) {
  ...
  async buildInputs(text, options = {}) {
    ...
    const phonemeTimeline = await this.createPhonemeTimeline(text);
    const phonemeIds = this.collectPhonemeIds(phonemeTimeline);
    const ort = await import('onnxruntime-web');
